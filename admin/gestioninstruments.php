...
while ($instrument = $requete_instruments->fetch(PDO::FETCH_ASSOC)) {
    ...
    $contenu .= '<div class="card border-dark col-3 container-fluid">';

    $contenu .= '<div class="row g-0">';

    $contenu .= '<div class="col-sm-4">';
    $contenu .= '<img class="card-img-top  h-auto" src="../img/triangles/' . $instrument['photo_produit_1'] . '" alt="' . $instrument['nom_produit'] . '">';
    $contenu .= '</div>';
    ...
    $contenu .= '<a href="formproduit.php?id=' . $instrument['id_produit'] . '" class="justify-contenu-center btn btn-danger" target="_blank">Voir le produit</a>';
    $contenu .= '</div>';
    $contenu .= '</div>';
