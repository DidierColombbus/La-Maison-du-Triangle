...
if ($nbcours != 0) {
  ...
  $contenu .= '<h1 class="bg-light"> ლ(╹◡╹ლ) Super, c\'est l\'heure des cours !</h1><br>';
  $contenu .= '<div class="card border-dark w-75 text-start">';

  $contenu .= '<div class="row g-0">';

  $contenu .= '<div class="col-sm-12">';
  $contenu .= '<div class="card-body">';
  $contenu .= '<h4 class="card-title">' . $cours['titre_cours'] . '</h4>';
  $contenu .= '<p class="card-text">' . $cours['description_cours'] . '</p>';
  ...
  $contenu .= '<div  class="bg-light container py-1">';
  $contenu .= '<h1 class="bg-light"> ¯\_(ツ)_/¯ Pas encore disponible malheureusement !</h1><br>';
  $contenu .= '<p class="bg-light">Nous sommes désolé, mais aucun cours n\'est encore ouvert actuellement. Vous pouvez nous contacter ici : <a href="contact.php">page contact</a> pour de plus amples informations à ce sujet.</p>';
  $contenu .= '<p class="bg-light">Ces cours vous seront proposés dés que possible, et ils permettront à tout un chacun, débutant comme confirmé, de pratiquer le triangle en toute sérénité et bienveillance.</p>';
  $contenu .= '</div>';
}
